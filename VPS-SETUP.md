...
- [x] add the public metadata-only registry and its strict schema;
- [x] complete the first read-only Atlas metadata audit. It found the four
      platform files and two Surplasse database passwords, for six materialized
      files in total;
...
- [ ] create a production age identity and store its private key in an approved
      secret manager with an offline recovery copy;
- [ ] prove age recovery, define `.sops.yaml`, and add encrypted files separated
      by rights. SOPS remains blocked until this proof exists. Set registry
      recovery to `verified` only after a complete restore exercise;
- [ ] create a read-only GHCR token if a private image requires it;
- [ ] document a read-only Git deploy key only if `vps-infra` becomes private;
- [ ] keep the GitHub trigger SSH key separate from every other identity.
