...
DIGEST_D = "4" * 64
DIGEST_E = "5" * 64
SURPLASSE_SMTP_READINESS_GATES = frozenset(
    {
...
class ReleasePolicyTests(unittest.TestCase):
    ...
    def test_surplasse_smtp_readiness_gates_are_declared_consistently(self) -> None:
        manifest = sample_manifest()
        adapter = json.loads(
        ...
        declarations = {
            "release policy": RELEASE_POLICY.SURPLASSE_READINESS_GATES,
            "Surplasse adapter": adapter["blocked_by"],
            "production release": manifest["applications"]["surplasse"]["blocked_by"],
        }

        for label, gates in declarations.items():
            with self.subTest(label=label):
                self.assertTrue(
                    SURPLASSE_SMTP_READINESS_GATES.issubset(set(gates)),
                    f"{label} is missing required Surplasse SMTP readiness gates",
                )
