...
class ApplicationControllerTests(unittest.TestCase):
    ...
    def test_recovery_removes_only_exact_monflorian_edge_staging_residue(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            app_root = root / "monflorian"
            releases = app_root / "releases"
            edge_releases = app_root / "edge-releases"
            releases.mkdir(parents=True)
            edge_releases.mkdir()
            app_root.chmod(0o755)
            releases.chmod(0o755)
            edge_releases.chmod(0o755)
            private_staging = edge_releases / f".sha256-{'a' * 64}-{'b' * 16}"
            private_staging.mkdir(mode=0o700)
            (private_staging / "partial").write_bytes(b"partial")
            exposed_staging = edge_releases / f".sha256-{'a' * 64}-{'c' * 16}"
            exposed_staging.mkdir(mode=0o700)
            exposed_staging.chmod(0o755)
            outside_pattern = edge_releases / f".sha256-{'a' * 64}-{'b' * 15}"
            outside_pattern.mkdir(mode=0o700)
            final = edge_releases / f"sha256-{'a' * 64}"
            final.mkdir(mode=0o755)
            with (
                mock.patch.object(CONTROLLER, "APPLICATION_ROOT", root),
                mock.patch.object(
                    CONTROLLER,
                    "MONFLORIAN_EDGE_RELEASE_ROOT",
                    edge_releases,
            ...
            self.assertFalse(private_staging.exists())
            self.assertFalse(exposed_staging.exists())
            self.assertTrue(outside_pattern.is_dir())
            self.assertTrue(final.is_dir())

    def test_recovery_refuses_a_monflorian_edge_staging_symlink(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            app_root = root / "monflorian"
            releases = app_root / "releases"
            ...
            outside = root / "outside"
            outside.mkdir()
            staging = edge_releases / f".sha256-{'a' * 64}-{'b' * 16}"
            staging.symlink_to(outside, target_is_directory=True)
            with (
                mock.patch.object(CONTROLLER, "APPLICATION_ROOT", root),
                mock.patch.object(
                ...
                with self.assertRaisesRegex(
                    CONTROLLER.ApplicationDeploymentError,
                    "Mon Florian edge release staging residue",
                ):
                    CONTROLLER.cleanup_application_filesystem_residue(
                        "monflorian",
                        expected_uid=os.geteuid(),
    ...
    def test_recovery_refuses_a_monflorian_edge_release_root_symlink(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            app_root = root / "monflorian"
            releases = app_root / "releases"
            ...
            with (
                ...
                with self.assertRaisesRegex(
                    CONTROLLER.ApplicationDeploymentError,
                    "cannot open Mon Florian edge release root safely",
                ):
                    CONTROLLER.cleanup_application_filesystem_residue(
                        "monflorian",
                        expected_uid=os.geteuid(),
    ...
    def test_recovery_bounds_monflorian_edge_staging_cleanup(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            app_root = root / "monflorian"
            releases = app_root / "releases"
            ...
            releases.chmod(0o755)
            edge_releases.chmod(0o755)
            for index in range(CONTROLLER.MAX_STAGING_RELEASES + 1):
                staging = edge_releases / (
                    f".sha256-{'a' * 64}-{index:016x}"
                )
                staging.mkdir(mode=0o700)
            with (
                mock.patch.object(CONTROLLER, "APPLICATION_ROOT", root),
                ...
                with self.assertRaisesRegex(
                    CONTROLLER.ApplicationDeploymentError,
                    "Mon Florian edge release cleanup exceeded its staging limit",
                ):
                    CONTROLLER.cleanup_application_filesystem_residue(
                        "monflorian",
                        expected_uid=os.geteuid(),
            ...
            self.assertEqual(
                len(list(edge_releases.iterdir())),
                CONTROLLER.MAX_STAGING_RELEASES + 1,
            )
