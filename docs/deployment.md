...
are separate assertions in the temporary Caddy probe and remain part of the
future public smoke gate.
...
The local `local_certs` probe is not a public TLS probe. The future live
applicator must still run strict public TLS checks after activation, restore the
previous symlink and quarantine the digest if that public check fails. No
current reconciliation path can invoke the materializer because the production
policy stays locked and `apply-release` is absent. A future workflow will
